...
library ClaimCommons {
    enum ClaimType {IsBalanceAuthority, IsMeteringAuthority, IsPhysicalAssetAuthority, MeteringClaim, BalanceClaim, ExistenceClaim, MaxPowerGenerationClaim, GenerationTypeClaim, LocationClaim, IdentityContractFactoryClaim, EnergyTokenContractClaim, MarketRulesClaim, AcceptedDistributorClaim, IsPeriodBlockOracleClaim }
    ...
    function claimType2Topic(ClaimType _claimType) public pure returns (uint256 __topic) {
        ...
        if(_claimType == ClaimType.IsPeriodBlockOracleClaim) {
            return 10130;
        }
    ...
    function topic2ClaimType(uint256 _topic) public pure returns (ClaimType __claimType) {
        ...
        if(_topic == 10130) {
            return ClaimType.IsPeriodBlockOracleClaim;
        }
